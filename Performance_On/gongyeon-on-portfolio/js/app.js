...
function setActivePage(pageName) {
  ...
  $$('[data-page]').forEach((button) => {
    if (button.classList.contains("nav-item") || button.classList.contains("top-nav-item")) {
...
function renderEvents() {
  ...
  $("#performanceList").innerHTML = events.map(eventCard).join("");
  $("#emptyList").classList.toggle("hidden", events.length > 0);
  $("#noticeGrid").innerHTML = getEvents().map(noticeCard).join("");

  $("#totalCount").textContent = getEvents().length;
  const urgentCount = getEvents().filter((event) => daysLeft(event.deadline) >= 0 && daysLeft(event.deadline) <= 3).length;
  $("#urgentCount").textContent = urgentCount;
  $("#urgentCountSide").textContent = `${urgentCount}개`;
...
function renderApplications() {
  $("#applicationList").innerHTML = applications.map(applicationCard).join("");
  $("#emptyApplications").classList.toggle("hidden", applications.length > 0);
  $("#applicationCount").textContent = applications.length;
  bindDynamicButtons();
}
...
function initForms() {
  $("#applyEvent").addEventListener("change", updateApplyInfo);
...
function initNavigation() {
  ...
  ["#searchInput", "#categoryFilter", "#regionFilter"].forEach((selector) => {
    $(selector).addEventListener("input", renderEvents);
    $(selector).addEventListener("change", renderEvents);
  });
